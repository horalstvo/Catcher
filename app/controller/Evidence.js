Ext.define('catcher.controller.Evidence', {
  ...
  updatePlayer: function(){
    ...
    // aktualizace dat o upravovaném hráči
		...
		var title = "Upraveno";
		var message = "Záznam upraven";
// 	
		
		Ext.Viewport.setMasked({xtype:'loadmask',message:'Aktualizuji databázi hráčů'});
	...
	showPlayer: function(list, record){
		...
		Ext.getStore("Teams").each(function(radek){				
			teams.push({text:radek.get("name_full"),value:radek.get("team_id")});
		});
  ...
  sestavEvidenci: function(team_id){        
    ...
    players.clearFilter();                                
		...
		teams.each(function(radek){						
			var team = {text:radek.get("name_full"),team_id:radek.get("team_id"),items: new Array};			
			data.push(team);
    ...
    players.clearFilter();			                               									
		...
		evidence.setData({text:"Hráči",items:data});		
